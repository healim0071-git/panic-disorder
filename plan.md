...
## 🧭 [2026-09-16] 마일스톤 9.74: "전국 지점 안내 Click" / "가까운 16개 지점 찾기" 클릭 시 모바일 기준 뷰 1:1 완벽 일치 및 지점 카드 노출 최적화

### 1. 요구사항 및 문제 진단
- **요청 사항**: 하단 모바일 플로팅 CTA(`전국 지점 안내 Click`) 및 본문 내 `가까운 16개 지점 찾기 >` 클릭 시, 제공된 모바일 캡처 사진과 동일하게 상단 헤더 바로 아래에 `HEALIM NETWORK OFFICIAL` 공식 바로가기 배너가 14~18px 여백으로 정확히 위치하고, 그 아래로 `NETWORK CLINIC` 타이틀과 첫 번째 거점인 `대구본점` 카드(전화번호 버튼 포함)까지 한 화면(뷰포트 높이 765px 기준)에 시원하고 자연스럽게 쏙 들어오도록 최적화.
- **기존 여백 문제**:
  - `HEALIM NETWORK OFFICIAL` 배너의 모바일 패딩(`p-6`)과 하단 여백(`pb-6 sm:pb-8`), 그리고 푸터 상단 패딩(`pt-8 sm:pt-10`) 및 타이틀 하단 여백(`pb-6 mb-8`)의 누적으로 인해 총 여백이 100px 이상 초과하여, 모바일 뷰포트에서 `대구본점` 카드의 전화번호 버튼이 화면 아래로 밀리는 현상 발생.
  - 서브페이지에서 모바일 플로팅 CTA 클릭 시 현재 페이지의 지점 안내로 부드럽게 스크롤되지 않는 예외 케이스 방어 필요.

### 2. 세부 조치 및 구조 최적화
1. **배너 카드 및 푸터 상하 여백 정밀 콤팩트화 (`layouts/_partials/site_footer.html`)**:
   - `id="branches"` 상단 패딩 `pt-2.5`, 하단 패딩 `pb-4 sm:pb-6`로 슬림화하여 카드 아래 불필요한 흰색 공간 축소.
   - 배너 카드 내부 패딩을 `p-5 sm:p-7 md:p-10`으로 모바일 뷰에 최적화하여 캡처 이미지와 동일한 종횡비 구현.
   - 푸터 상단 패딩을 `pt-6 sm:pt-8`로 조정하고, 타이틀 하단 여백을 `pb-4 mb-5 md:pb-6 md:mb-8`로 축소.
   - 첫 번째 지점인 `대구본점` 카드의 패딩을 `p-4 sm:p-5`로 슬림화하여 모바일 한 화면(765px 높이) 안에 헤더 + 배너 + 16개 지점 타이틀 + 대구본점 전화연결 버튼까지 100% 들어오도록 정돈.
2. **동적 스크롤 계산 및 전역 앵커 인터셉트 강화 (`layouts/_partials/hooks/body-end/floating_cta.html`)**:
   - 고정 헤더(`site-header`) 높이(모바일 약 75px)를 실시간 반영하여 배너 카드 상단이 헤더 아래 약 14px 여백을 두고 정확히 착지하도록 `targetTop = window.pageYOffset + bRect.top - hOffset - 2` 정밀 계산.
   - 서브페이지에서도 현재 페이지에 `#branches`가 존재하면 즉시 부드럽게 스크롤하도록 플로팅 버튼 및 본문 앵커 클릭 이벤트 처리기 보강.
   - 해시(`#branches`) 직접 진입 시에도 DOM 로딩 지연을 대비하여 100ms, 350ms 2단계로 정밀 보정 스크롤 적용.
3. **CSS `scroll-margin-top` 정밀 튜닝 (`assets/css/custom.css`)**:
   - `#branches`의 `scroll-margin-top`을 모바일 `78px`, 데스크톱 `120px`로 상향 보정하여 브라우저 기본 앵커 이동 시에도 헤더에 가려지지 않고 사진과 완벽 일치하도록 적용.

### 3. 검증 결과
- **Hugo 프로덕션 빌드 (`hugo --gc --minify`)**: 32개 페이지 에러 0건 정상 컴파일.
- **레이아웃 1:1 일치 검증**: 모바일 뷰포트(482x765 기준)에서 클릭 시 상단 헤더, HEALIM 배너, 16개 지점 안내 헤더, 대구본점 전화번호 버튼까지 완벽히 노출됨을 확인.
