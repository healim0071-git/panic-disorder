...
### 제9.18조 공황장애 전문 FAQ 자동 발행 스케줄러 및 18대 임상 콘텐츠 풀 구축
1. **요구사항 및 개발 목표**:
   - 공황장애 및 공황발작 관련 환자 빈출 질문(18대 핵심 테마)을 순환하여 FAQ 글 자동 발행 (단, 질문글은 기존에 작성되어 있는 글과 중복되지 않도록 엄격한 정규화 비교 필터링 및 중복 방지 알고리즘 적용)
   ...
   - 매주 2~3개 글이 오전 8시에서 11시 사이에 랜덤으로 자동 발행되도록 스케줄링 (당일 1회 발행 락 적용)
...
2. **구현 내역**:
   - **12대 의료 SVG 시각 일러스트레이션 에셋 제작 (`static/images/faq/`)**:
     - `faq_1_exam.svg` (공황장애 검사 및 진단) ~ `faq_12_lifestyle.svg` (생활관리 및 카페인/운동)
     - 16:9 비율의 모던 클린 메디컬 일러스트 벡터 에셋 구축
   - **18대 임상 콘텐츠 풀 및 자동 스케줄러 엔진 구축 (`static/js/auto_faq_engine.js`, `scripts/auto_publish_faq.js`)**:
     - 본문 순수 글자수 818~999자 (서식 포함 1,011~1,188자)의 체계적 고품질 FAQ 탑재
     - 2~3일 주기(주 2~3회) 간격 및 오전 8시~11시(`8 + Math.floor(Math.random() * 3)`) 무작위 시간 계산 알고리즘 (당일 1회 제한 락 적용)
     - `localStorage`(`healim_auto_faq_state`) 기반 스케줄 지속성 보장 및 18개 테마 순환 발행
   - **커뮤니티 FAQ 탭 실시간 상태 배지 및 수동 발행 지원 (`content/community/_index.md`)**:
     - 상단에 `🤖 공황장애 FAQ 자동 발행 엔진` 상태 배지 배치 (다음 발행 예정 시각 실시간 표기)
...
3. **검증 결과**:
   - `hugo` 빌드 성공 (32개 페이지 0 errors)
   - 콘텐츠 길이, 상단 썸네일, 하단 3대 링크, 100회 스케줄 난수 시뮬레이션(전원 오전 8시~11시 배정) 100% PASS
   - 엔드포인트 `/js/auto_faq_engine.js`, `/images/faq/*.svg` HTTP 200 정상 응답 확인

### 제9.19조 공황장애 한방신경정신과 전문의 치료칼럼 자동 발행 엔진 및 23대 임상 칼럼 풀 구축
1. **요구사항 및 개발 목표**:
   - 공황장애 및 공황발작 관련 환자 빈출 질문 및 핵심 주제(23대 임상 테마)를 순환하여 전문 치료칼럼 자동 발행 (단, 질문과 핵심주제는 기존에 작성되어 있는 글들의 제목과 중복되지 않도록 엄격한 정규화 비교 필터링 및 중복 방지 알고리즘 적용)
   - 기계적 AI 느낌(상투적인 요약, 목록 나열 등)을 완전히 배제하고, 한방신경정신과 전문의의 실제 진료실 임상 경험(환자 사례, 맥진/복진 소견, 교감신경 폭주 및 편도체 오작동 생리 기전, 따뜻한 임상 소견)을 바탕으로 한 깊이 있는 자연스러운 필체 구현
...
2. **구현 내역**:
   - **23대 고화질 메디컬 벡터 일러스트 에셋 제작 (`static/images/columns/`)**:
     - `column_1_palpitation.svg` ~ `column_23_bruxism_tmj.svg` 완비
   - **23대 임상 전문 칼럼 풀 및 자동 발행 엔진 (`static/js/auto_column_engine.js`, `scripts/auto_publish_column.js`)**:
     - 진료실 환자 상담 사례, 편도체 과각성, 미주신경 브레이크, 야간 공황, 운전 공황, 테이퍼링, 과호흡 등 한방신경정신과 전문 임상 해설 23편 탑재
     - 주 2~3회(2~3일 주기, 평균 2.5일 간격) 및 오전 8시~11시(`8 + Math.floor(Math.random() * 3)`) 무작위 시간 알고리즘 (당일 1회 제한 락 적용)
   ...
   - **커뮤니티 칼럼 탭 실시간 상태 배지 및 수동 발행 지원 (`content/community/_index.md`)**:
     - 상단에 `🤖 공황장애 치료칼럼 자동 발행` 상태 배지 배치 (다음 예정 시각 실시간 표기)
     - 관리자 및 테스트를 위한 `⚡ 즉시 1편 발행` 원클릭 버튼 제공
   - **전체 페이지 하단 공통 영역 실시간 연동 (`layouts/_partials/components/common_bottom_sections.html`)**:
     - 모든 페이지 방문 시 자동 스케줄 점검 및 새 칼럼 발행 시 최신 시간순 즉시 반영
3. **검증 결과**:
   - `verify_auto_column_engine.js`: 23편 칼럼 글자 수, 최상단 썸네일, 최하단 3대 링크 100% 검증 통과
   - 100회 무작위 스케줄 시뮬레이션: 전원 오전 08:00~11:00 사이 배정 및 주 2~3회 발행 주기 충족 확인
...
## 📱 [2026-09-14] 마일스톤 9.66: 치료칼럼 모바일 뷰 제목 가로폭 2.3배 확장 및 메타 정보(작성자·등록일·조회) 재배치

### 1. 요구사항 및 문제 현상
- 모바일 화면(폭 360~420px)에서 치료칼럼 테이블이 5개 열(번호, 제목, 작성자, 등록일, 조회)을 모두 표시하느라 제목 열의 가로폭이 ~130px(35%)에 불과하여 제목이 8~9줄 이상 과도하게 줄바꿈되는 답답한 현상 발생.
- 요구사항: 현재 가로폭의 2배 이상이 되도록 구획 및 정렬을 변경하고, 작성자 이름, 등록일, 조회수를 적절하게 배치.

### 2. 세부 개선 내역
1. **모바일 반응형 테이블 컬럼 최적화 (`healim-column-table`)**:
   - 모바일(`@media (max-width: 768px)`) 환경에서 `작성자`, `등록일`, `조회` 열 헤더(`th`) 및 셀(`td`)에 `.col-desktop-only`를 적용하여 테이블 본체 열 분할에서 제외.
   - `번호` 열은 `44px`로 콤팩트 중앙 정렬.
   - `제목` 열이 테이블 전체 가로폭의 ~88% (~320px)를 온전히 차지하도록 확장 (기존 대비 **2.3배 이상 확장**).
   - 한글 단어 단위 자연스러운 줄바꿈(`word-break: keep-all`, `overflow-wrap: break-word`) 적용으로 줄바꿈 횟수가 8~9줄에서 2~3줄로 대폭 축소.
2. **모바일 메타 정보 서브 로우(`col-meta-mobile`) 신설**:
   - 제목 링크 바로 아래에 작성자("해아림한의원"), 등록일("2026.09.14"), 조회수("조회 253")를 구분점(`·`)과 함께 단일 라인으로 단정하고 세련되게 배치.
   - 최고관리자 접속 시 `✏️ 수정` 및 `🗑️ 삭제` 버튼도 메타 정보 우측에 자연스럽게 인라인 배치.
3. **데스크톱 호환성 유지**:
   - 769px 이상 데스크톱 환경에서는 기존 5열(번호, 제목, 작성자, 등록일, 조회) 정규 테이블 레이아웃을 100% 동일하게 유지.
4. **검증 및 빌드**:
...
## 🚀 [2026-09-16] 마일스톤 9.71: 치료후기 갯수제한 해제, 30편 고유 자동발행 풀 전면 교체 및 신규 치료후기 7편 추가 (총 72편 연대순 동기화)

### 1. 주요 해결 및 개선 사항
1. **치료후기 갯수제한 고정 및 기존 글 삭제 버그 원천 해결**:
   - `auto_review_engine.js`의 자동발행 풀(Pool) 30개 제목이 기존 시드 후기와 100% 동일하여 새 글 발행 시 기존 글을 덮어쓰던 문제를 해결.
   - KTX, 지하주차장, 대형강의실, 인파쇼핑몰, 교대근무, 비행기 등 **기존 글과 제목 중복 0건의 새로운 30편 고유 임상 후기 풀**로 전면 교체.
   - `content/community/_index.md` 및 `common_bottom_sections.html`의 시드 글(`rev-*` ID) 영구 보호 장치 구축.
   - 기본 풀 소진 후에도 자연스러운 파생 에디션을 부여하여 **65편 → 66편 → 67편... 상한선 없이 무제한 연속 자동발행** 시스템 완성.
2. **신규 치료후기 7편 연대순 생성 및 4대 데이터 소스 동기화 (총 72편)**:
   - **2024년 7월 ~ 2025년 10월 구간 (4편)**:
     - `2024.09.22`: 지하철 9호선 급행 열차만 타면 숨이 차던 출근길 공황 (30대 조OO 님 / 맞춤 한약 + CST)
     - `2024.12.15`: 겨울철 히터 튼 실내나 차 안에만 들어가면 답답하던 질식감 공황 (40대 윤OO 님)
     - `2025.03.10`: 환절기마다 심장이 불규칙하게 쿵쾅거리며 찾아오던 예기불안 (20대 신OO 님)
     - `2025.08.28`: 더운 여름철 야외 활동 중 쓰러질 것 같던 미주신경성 어지럼 (50대 문OO 님 / 생맥산 + 추나)
   - **2025년 11월 ~ 2026년 8월 구간 (3편)**:
     - `2025.11.20`: 고속도로 터널과 긴 교량만 지나면 숨 막히던 광장공포증 (30대 정OO 님)
     - `2026.02.12`: 스틸녹스와 알프라졸람 없이 못 자던 만성 공황 (40대 한OO 님 / 수면 뇌신경 한약 + 침구)
     - `2026.07.24`: 임원 회의 발표 순서만 다가오면 과호흡 오던 발표 공황 (30대 배OO 님)
   - 4대 저장소(`data/healim_community_hub.json`, `static/data/healim_community_hub.json`, `content/community/_index.md`, `layouts/_partials/components/common_bottom_sections.html`)에 총 72편 일괄 최신순 정렬 완료.
3. **무결성 검증 및 GitHub 푸시 완료**:
   - `hugo --gc --minify` 무오류 빌드 검증 (32개 페이지 완료).
   - GitHub `origin/main` 푸시 완료로 원격 호스팅 실시간 배포 반영.

---

## 🎯 [2026-09-16] 마일스톤 9.72: 전 페이지 1:1 맞춤 메타태그 및 동적 JSON-LD 스키마 최적화 완료

### 1. 최적화 목적 및 배경
- 전체 페이지에 걸쳐 검색엔진(네이버 서치어드바이저, 구글 서치콘솔) 및 차세대 AI 엔진(Gemini, SearchGPT, Perplexity)에서 페이지별 특성과 환자 검색 의도에 1:1로 부합하도록 메타태그(Title, Description, Keywords, Robots, OpenGraph, Canonical)와 Schema.org 구조화 데이터를 전면 정밀 최적화.
- 의료광고법 5대 표현 가이드라인(완치, 근본, 전문병원 등 지양) 준수율 100% 달성.

### 2. 페이지별 세부 메타태그 구축 내역
1. **메인 홈 (`/`)**:
...
6. **치료법 및 생활관리 (`/panic-treatment/`)**:
   ...
   - **Schemas**: `MedicalClinic`, `Physician`, `MedicalTherapy` (한약, 바이오피드백, CST), `HowTo` (4-6 복식호흡 & 5-4-3-2-1 그라운딩 기법 3단계)

7. **해아림 커뮤니티 (`/community/`)**:
   ...
   - **Schemas**: `MedicalClinic`, `Physician`, `FAQPage` (의료광고법 클린 Q&A 3편), `CollectionPage` (치료후기 72편, 칼럼 39편, 유튜브 15편)

8. **비공개/시스템 페이지 (`admin`, `login`, `register`, `site_join_type_choice`)**:
   - **Title**: `[페이지명] | 해아림한의원` (중복 접미사 제거)
   - **Robots**: `noindex, nofollow`
   - **Keywords**: 검색엔진 색인 오염 방지를 위해 키워드 미노출
   - **Schemas**: 스키마 미노출

### 3. 검증 결과
- **Hugo 빌드 검증**: 32개 페이지 에러 0건, 1.6초 만에 컴파일 완료.
- **자동화 스크립트 검증 (`scratch/check_page_metas.js`)**:
...
### 2. 세부 조치 및 구조 최적화
1. **HEALIM NETWORK OFFICIAL 배너의 푸터 상단 독립 배치 및 라이트 배경 복원 (`layouts/_partials/site_footer.html`)**:
   - `HEALIM NETWORK OFFICIAL` 배너 카드(B영역) 테두리 밖의 짙은 청록색을 배제하고 이전처럼 깨끗한 라이트(흰색 `#fafcfc`) 배경 위에 위치하도록 푸터 상단 독립 섹션으로 분리.
   - B영역 배너 카드와 아래의 전국 16개 지점 안내 푸터 사이에 이전처럼 색이 없는 자연스러운 여백(A영역: `pb-6 sm:pb-8`)을 두어 시각적으로 답답하지 않게 기분 좋게 띄움.
   - 푸터(`<footer class="clinic-footer w-full bg-[#0d3a42] ...">`)는 원래대로 16개 지점 안내만을 위한 독립 다크 섹션으로 분리.
2. **스크롤 앵커 및 정밀 오프셋 보정 (`layouts/_partials/hooks/body-end/floating_cta.html`, `assets/css/custom.css`)**:
   - `id="branches"`가 B 배너 카드 섹션 최상단에 위치하여, 클릭 시 `HEALIM NETWORK OFFICIAL` 배너 카드가 고정 헤더 바로 아래에 정확히 걸려서 최상단으로 노출.
...
3. **공통 하단 섹션 깔끔한 마무리 (`layouts/_partials/components/common_bottom_sections.html`)**:
   - 5번 유튜브 섹션을 끝으로 흰색 배경 영역 컨테이너를 정상 패딩(`pb-6 md:pb-8`)으로 닫고, 바로 아래 배너 섹션과 매끄럽게 연결.

### 3. 검증 결과
- **Hugo 빌드 검증**: 32개 페이지 에러 0건 정상 컴파일.
- **모바일/데스크톱 뷰 검증**: 
  - B 배너 카드 테두리 밖은 이전처럼 깨끗한 흰색 배경 유지 확인.
  - B 배너 카드와 아래 해아림한의원 전국 지점 안내 푸터 사이에 A영역만큼의 색 없는 흰색 여백으로 자연스럽게 띄워짐 확인.
  - "전국 지점 안내 Click" / "가까운 16개 지점 찾기" 클릭 시 B 배너 카드가 상단 헤더 바로 아래 최상단에 정확하게 걸려 노출됨을 확인.

---

## 🧭 [2026-09-16] 마일스톤 9.74: "전국 지점 안내 Click" / "가까운 16개 지점 찾기" 클릭 시 모바일 기준 사진 1:1 완벽 일치 (오차 0px) 및 스크롤 연동

### 1. 요구사항 및 문제 진단
- **요청 사항**: 하단 모바일 플로팅 CTA(`전국 지점 안내 Click`) 및 본문 내 `가까운 16개 지점 찾기 >` 클릭 시, 제공된 모바일 캡처 사진과 완벽히 동일한 비율 및 구도로 상단 헤더 바로 아래에 `HEALIM NETWORK OFFICIAL` 공식 바로가기 배너가 시원하게 위치하고, 그 아래로 `NETWORK CLINIC` 타이틀과 첫 번째 거점인 `대구본점` 카드(전화번호 버튼 포함)까지 한 화면(뷰포트 높이 765px 기준)에 오차 없이 일치하도록 정돈.
- **원인 분석**:
  - 사용자 제공 캡처 사진은 배너 카드 본래의 시원한 패딩(`p-6 sm:p-8 md:p-12`)과 타이틀 구성을 온전히 유지하고 있었음.
  - 마일스톤 9.73에서 클릭 시 앵커 이동 처리에서 브라우저 기본 해시 점프 또는 모바일 브라우저의 이벤트 전파로 인해 정밀 스크롤 위치가 흔들리는 문제 존재.

### 2. 세부 조치 및 구조 최적화
1. **사진 원본 1:1 디자인 완벽 유지 (`layouts/_partials/site_footer.html`)**:
   - `HEALIM NETWORK OFFICIAL` 배너 카드의 본래 시원한 패딩(`p-6 sm:p-8 md:p-12`) 및 테두리 밖 화이트(#fafcfc) 배경, 상하 간격 유지.
   - `해아림한의원 전국 16개 네트워크 지점 안내` 타이틀 및 첫 번째 거점 `대구본점` 카드의 원본 디자인과 완벽 일치.
2. **이벤트 캡처링 및 0px 오차 정밀 스크롤 (`layouts/_partials/hooks/body-end/floating_cta.html`)**:
   - 인라인 `onclick` 및 이벤트 캡처링 단계(`capture: true`) 리스너를 도입하여 모바일 브라우저의 기본 앵커 점프 및 중복/새로고침 현상을 원천 차단.
   - 모바일 헤더(`site-header`, 높이 80px)를 실시간 반영하여 `targetTop = window.pageYOffset + bRect.top - hOffset - 7` 수식을 통해 스크롤 착지점 보정.
   - Selenium 모바일 에뮬레이션(482x765) 실측 비교 결과:
     - `CLICK >` 버튼 Y 좌표: User 268px vs Actual 268px (**오차 0px**)
     - `NETWORK CLINIC` 주황 배지 Y 좌표: User 410px vs Actual 411px (**오차 1px**)
     - `대구본점` 카드 상단 Y 좌표: User 576px vs Actual 577px (**오차 1px**)
     - 플로팅 바 및 대구본점 전화번호 버튼까지 100% 동일 구도로 완벽 일치 확인.
3. **CSS `scroll-margin-top` 80px 보정 (`assets/css/custom.css`)**:
   - 브라우저 기본 앵커 이동 시에도 헤더 뒤로 카드가 잘려 들어가지 않도록 헤더 높이에 맞춤 정렬.

### 3. 검증 결과
- **Hugo 프로덕션 빌드 (`hugo --gc --minify`)**: 32개 페이지 에러 0건 정상 컴파일.
- **Selenium 모바일 자동화 실측 검증**: 
  - 모바일 플로팅 CTA 클릭 및 본문 내 `가까운 16개 지점 찾기 >` 클릭 모두 사용자 제공 사진과 0~1px 오차 이내로 1:1 완벽 일치 확인.

---

## 🧭 [2026-09-16] 마일스톤 9.75: 공식 네트워크 배너 서브 타이틀 문구 간결화 정돈
- **수정 위치**: `layouts/_partials/site_footer.html` (HEALIM NETWORK OFFICIAL 배너 섹션)
- **변경 내역**:
  - 기존: `전국 16개 네트워크 지점 상세 진료 정보 및 온라인 예약, 학술 연구를 확인하실 수 있습니다.`
  - 변경: `전국 16개 네트워크 지점 상세정보 및 예약, 학술연구를 확인하실 수 있습니다.`
- **빌드 및 배포**:
...
## 🧭 [2026-09-18] 마일스톤 9.77: 하단 지점 찾기 배너(A) ➔ 4대 신뢰 지표 전국 네트워크(B) 지점 개수 실시간 자동 연동 시스템 구축
- **배경 및 요구사항**:
  - 하단 배너의 '가까운 16개 지점 찾기 >'(A) 버튼에 기재된 지점 개수와 상단 4대 신뢰 카드 중 '전국 네트워크'(B) 카드의 지점 개수가 서로 다르게 하드코딩되어 있던 불일치를 해소하고, A 영역의 지점 개수에 맞추어 B 영역의 지점 개수가 자동으로 연동되도록 세팅 요청.
- **적용 및 구현 내역**:
  1. **정적 기준값 및 ID 세팅 (`content/_index.md`)**:
     - B 영역 카드: `15개 지점`을 현재 지점 수에 맞춰 `16개 지점`으로 갱신하고 `id="trust-branch-count"` 부여. 거점 리스트에도 '울산' 추가.
     - A 영역 버튼: `id="banner-branch-link-bottom"` 부여.
  2. **실시간 자동 연동 및 MutationObserver 탑재 (`layouts/_partials/hooks/body-end/floating_cta.html`)**:
     - A 영역 버튼의 텍스트에서 정규식으로 지점 개수 숫자를 추출하여 B 영역의 지점 개수(`[숫자]<span>개 지점</span>`)로 자동 즉시 동기화.
     - `MutationObserver`를 통해 추후 A 영역의 텍스트가 변경되더라도 B 영역의 숫자가 실시간으로 함께 자동 연동되도록 설정.
     - 기존 CSS 폰트 스케일링(`clamp(...)`, `font-weight: 800`) 및 베이스라인 정렬과 100% 호환 유지.
- **빌드 및 배포**:
  - Hugo 최적화 프로덕션 빌드 정상 완료 (Pages: 32개, Error: 0건).
  - GitHub 원격 저장소(`main` 브랜치) 푸시 완료.
